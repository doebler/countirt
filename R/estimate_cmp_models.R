...
newem_estep2 <- function(data, item_params, weights_and_nodes, 
  ...
  if (include_poffset) {
    PPs <- e_values_poff_cpp(
      ...
      grid_mus = new_grid_mus,
      grid_nus = grid_nus,
      grid_logZ_long = new_grid_log_Z_long,
      grid_log_lambda_long = new_grid_log_lambda_long,
      max_mu = 300,
      min_mu = 0.001
    )
  } else {
...
grad_cmp_newem2 <- function(item_params, PPs, weights_and_nodes, data, 
  ...
  if (include_poffset) {
    grads <- grad_cmp_poff_cpp(
      ...
      grid_mus = new_grid_mus,
      grid_nus = grid_nus,
      grid_cmp_var_long = new_grid_cmp_var_long,
      grid_log_lambda_long = new_grid_log_lambda_long,
      grid_logZ_long = new_grid_log_Z_long,
      max_mu = 300,
      min_mu = 0.001)
...
grad_cmp_fixdisps_newem <- function(item_params, PPs, 
  ...
  if (include_poffset) {
    grads <- grad_cmp_fixdisps_poff_cpp(
      alphas = alphas, 
      deltas = deltas, 
      disps = disps, 
      item_offset = item_offset,
...
grad_cmp_fixalphas_newem <- function(item_params, PPs, 
  ...
  if (include_poffset) {
    grads <- grad_cmp_fixalphas_poff_cpp(
      alphas = alphas, 
      deltas = deltas, 
      disps = disps, 
      item_offset = item_offset,
...
grad_cmp_samedisps_newem <- function(item_params, PPs, 
  ...
  if (include_poffset) {
    grads <- grad_cmp_samedisps_poff_cpp(
      alphas = alphas, 
      deltas = deltas, 
      disps = disps, 
      item_offset = item_offset,
...
grad_cmp_samealphas_newem <- function(item_params, PPs, 
  ...
  if (include_poffset) {
    grads <- grad_cmp_samealphas_poff_cpp(
      alphas = alphas, 
      deltas = deltas, 
      disps = disps, 
      item_offset = item_offset,
...
# newem_em_cycle ---------------------------------------------------------------------
newem_em_cycle <- function(item_params, data, weights_and_nodes,
                           ctol_maxstep = 1e-8, m_method = "nleqslv",
                           fix_disps = NULL, fix_alphas = NULL,
                           same_disps = FALSE, same_alphas = FALSE,
                           item_offset = NULL, person_offset = NULL) {
  # I handled empty item_offset and person_offset arguments in run_newem, so that I can expect a vector
...
# run_newem ----------------------------------------------------------------------
run_newem <- function(init_params, data, n_nodes, thres = Inf, prob = 0,
...
get_start_values <- function(data, nodes = 121, nsim = 1000,
  ...
  if (!is.null(fix_disps)) {
    ...
    mean_person_offset <- mean(person_offset)
    init_logdisps<-c()
    sim_abilities=rnorm(nsim)
    for (i in 1:ncol(data)) {
      if (same_alpha) {
        mu <- exp(init_deltas[i] + init_alphas*sim_abilities + item_offset[i] + mean_person_offset)
      } else {
        mu <- exp(init_deltas[i] + init_alphas[i]*sim_abilities + item_offset[i] + mean_person_offset)
      }
      sim <- rpois(nsim, mu)
