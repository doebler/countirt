...
// [[Rcpp::export]]
NumericVector grad_poisson_with_icov_cpp(NumericVector alphas,
                                         NumericVector deltas,
                                         NumericVector betas,
                                         NumericMatrix data,
                                         NumericMatrix i_cov_data,
  ...
  for(int j=0;j<M;j++){
    ...
    for(int k=0;k<K;k++) {
      ...
      for(int i=0;i<N;i++) {
        ...
        // compute the gradients (summing over persons)
        grad_alphas[i] += PPs(i,k) * nodes[k]*(data(i,j) - mu);
        grad_deltas[i] += PPs(i,k) * (data(i,j) - mu);
      }
    }
  }
    ...
    for (int k=0;k<K;k++) {
      ...
      for (int i=0; i<N; i++) {
        ...
        for (int j=0; j<M; j++) {
          ...
          for(int d=0; d<I; d++) {
            // add all the (weighted) covariate values for all covariates
            log_mu += betas[d] * i_cov_data(j,d); // for item j
          }
          double mu = exp(log_mu);
          // over items (as the betas are only specific to item covariates, not items)
          grad_betas[c] += PPs(i,k) * i_cov_data(j,c) *(data(i,j) - mu);
        } // end loop over m (items)
      } // end loop of n_nodes
    } // end loop over P (person covariates)
  } // end loop over items
